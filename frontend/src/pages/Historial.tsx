import { useState } from "react";
  ...
  import { Search, Filter, Eye, Trash2, FileText, AlertCircle, Loader2 } from "lucide-react";
  import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
  import { apiListarInspecciones, apiEliminarInspeccion, apiGetInspeccion } from "@/lib/api";
  ...
  export default function Historial() {
    ...
    const handleDownloadPDF = async (id: number) => {
      setDownloadingId(id);
      try {
        const insp = await apiGetInspeccion(id);
                    ...
                    <tr key={insp.id} className="hover:bg-muted/30 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-center">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 hover:bg-primary/20 text-primary rounded-md text-xs font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                          title="Descargar PDF"
                        >
